 ...
 const submitAnswers = async () => {
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      // 💾 Save to DB immediately after each feedback is generated
      const saveRes = await saveAnswerToDatabase({
  question: question.question,
  userAnswer,
...
const answerPayload = {
      ...
      if (saveRes.err) {
        console.warn(`Failed to save answer for Q${i + 1}:`, saveRes.message);
      }
    }
  ...
  const parseFeedback = (text) => {
    ...
    lines.forEach((line) => {
      if (line.startsWith("Score:")) {
        ...
        feedback.improvementSuggestions = line
          .replace("Improvement Suggestions:", "")
          .trim();
      } else {
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text(
      `Technical Questions: ${category}${topic ? ` (${topic})` : ""}`,
      105,
      15,
      { align: "center" }
    );
    ...
    doc.save(
      `technical-questions-${new Date().toISOString().slice(0, 10)}.pdf`
    );
  };
      ...
      <div id="technical-page" className="page fade-in">
        ...
        {questions.length > 0 && (
          <div id="technical-questions-container">
            ...
            {showFeedback && (
              <div
                id="technical-overall-feedback"
                className="feedback-card mt-6"
              >
                <div className="feedback-score">
                  {t("Overall Score")}:{" "}
                  {Math.round(
                    feedback.reduce((sum, fb) => sum + fb.score, 0) /
                      questions.length
                  )}
                  /10
                </div>
                <div className="feedback-text">
                  {feedback[0]?.score >= 8
                    ? t(
                        "Excellent! You demonstrated strong technical knowledge."
                      )
                    : feedback[0]?.score >= 5
                    ? t(
                        "Good job! You have a solid understanding but room for improvement."
                      )
                    : t(
                        "Keep practicing! Review the feedback to identify areas to focus on."
                      )}
                </div>
                  ...
                  {questions.map((question, index) => (
                    ...
                    >
                      ...
                      <div className="answer-display mt-2">
                        {t("Your answer")}:{" "}
                        {answers[index] || t("No answer provided")}
                      </div>
                      <p
